...
const setBgColor = (currentStep: number, idx: number) => {
  const bgColor =
    currentStep < idx ? `bg-${StepperColors['inActive']}` : `bg-${StepperColors['active']}`;
...
const setTextColor = (currentStep: number, idx: number) => {
  const textColor =
    currentStep < idx ? `text-${StepperColors['inActive']}` : `text-${StepperColors['active']}`;
...
const setWidth = (currentStep: number, idx: number) => {
  if (currentStep == idx) {
    return `w-[${Progresswidth['half']}]`;
  } else if (currentStep < idx) {
    return `w-[${Progresswidth['full']}]`;
  } else {
    return `w-[${Progresswidth['full']}]`;
  }
};
